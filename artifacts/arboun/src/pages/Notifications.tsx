...
const notifications = [
  ...
  { icon: "🔔", bg: "hsl(var(--secondary))", label: "تذكير: مدة الحجز", sub: "تبقّى 12 يوماً على صفقة حي النرجس", time: "قبل 3 أيام" },
];
        ...
        {notifications.map((n, i) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[13.5px] font-bold truncate" style={{ color: "hsl(var(--foreground))" }}>{n.label}</p>
              <p className="text-[11px] mt-0.5 truncate" style={{ color: "hsl(var(--muted-foreground))" }}>{n.sub}</p>
            </div>
            <span className="text-[11px] flex-shrink-0" style={{ color: "hsl(var(--muted-foreground))" }}>{n.time}</span>
          </div>
        ))}
      </div>
    </Layout>
